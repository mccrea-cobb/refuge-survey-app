...
derived <- dat %>%
  ...
  select(StationName, SurveyName, isSurveyConducted, scientificName, commonName)
...
bio1 <- bio1 %>%
  count(isSurveyConducted, scientificName, commonName) %>%
...
```{r biotic-grp2, echo=F, message=F}
...
In `r year_selected`, refuges in the `r region` Region collected data on `r sum(completed$n)` surveys of `r sum(scheduled$n)` scheduled surveys (`r scales::label_percent()(sum(completed$n) / sum(incomplete$n))`) (Table \@ref(tab:tbl-activity)). The most commonly reported reason for why data were not collected for `r sum(incomplete$n)` surveys was `r tolower(dat_reasons$reason[1])` [`r dat_reasons$n[1]`] challenges, followed by issues associated with a `r tolower(dat_reasons$reason[2])` [`r dat_reasons$n[2]`] (Table \@ref(tab:tbl-reasons)). Based on the comments associated with the reasons listed, COVID-19 restrictions prevented staff from collecting data on `r covid_comments` scheduled surveys.

In `r year_selected`, refuge scientists collected survey data on a variety of biotic groups and species. The most surveyed biotic group (n = `r bio1$n[[1]]`) was `r as.character(bio1$bio_grp[[1]])` (Figure \@ref(fig:plot-bio-grp1)). The most surveyed biotic level 2 group (n = `r bio2$n[[1]]`) was `r as.character(bio2$bio_grp[[1]])` (Table 3). The most surveyed species (n = `r spp$n[[1]]`) was `r as.character(spp$species[[1]])` (Table 4).
